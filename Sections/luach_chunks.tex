...
\chanukkaWeekdayMorning{\leinChanukahDayI}
}
...
\d{H}anukka candles are lit in shul, without a berahka (#3 candles). Sha\d{h}arit #1 morning includes \textit{ya`ale veyavo} and \textit{`al hanisim} in the Amidah.  After the leader recites the communal amidah, full Hallel is recited.  \fullkad\space is said, since Musaf will be recited.  Two Sifrei Torah are taken out, without \textit{`el `erekh `apayim}.
