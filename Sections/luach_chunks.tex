...
Sha\d{h}arit #1 morning includes \textit{`al hanisim} in the Amidah.  After the leader recites the communal amidah, Hallel is recited (no ta\d{h}anun).  \halfkad\space is said, since there is no Musaf today.  The Torah is taken out, without \textit{`el `erekh `apayim}.  Three aliyot are read as follows:
...
\newcommand{\kislevTues}{%not including hanukka
